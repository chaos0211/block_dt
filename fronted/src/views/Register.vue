  ...
  <div class="min-h-[calc(100vh-64px)] flex items-center justify-center bg-gray-50 p-4">
    <div class="card p-6 max-w-md w-full">
      ...
      <form class="space-y-3" @submit.prevent="onSubmit">
        ...
        <input v-model="email" type="text" placeholder="邮箱" class="w-full border rounded px-3 py-2" />
        <div class="w-full">
          <select v-model="isAdmin" class="w-full border rounded px-3 py-2 text-sm">
...
async function onSubmit() {
  if (!username.value || !email.value || !pwd.value || !confirm.value) {
    alert("请完整填写必填项");
    return;
  }
